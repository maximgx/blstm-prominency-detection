...
\section{Introduzione}
	...
	quale fenomeno prosodico viene trattato
	quale metodo viene adottato (machine learning)
	
	corpus
	metodi di classificazione usati in passato
	cosa voglio usare ora
	organizzazione del paper

	introduzione
	prominenza, classificazione lingue e definizione
	metodi di rilevamento automatico, tipi di paradigma, supervised/not, corpus

\section{Prominenza Prosodica}\label{sec:prom}
	Le lingue umane sono classificabili in diverse classi di equivalenza. Il \textit{pitch} è una particolare frequenza del suono, detto anche \textit{tono}. Nella classe di equvalenza delle \textit{lingue tonali}, le differenze nei profili del pitch sono fondamentali per distinguere i diversi significati della stessa parola. Un esempio di lingua tonale è il tailandese. D'altra parte, si hanno le \textit{stress languages}, nelle quali il tono non incide in alcun modo sui significati lessicali delle parole (l'inglese per esempio). È in questa classe che, come è stato detto prima, certe sillabe vengono \textit{stressate} in funzione comunicativa. Una via di mezzo sono le lingue \textit{pitch-accented}, dove vi sono sì degli ancoraggi lessicali dei profili intonativi, ma con opzioni di configurazione molto ridotte. Verranno prese in considerazione le \textit{stress languages}, in quanto l'italiano appartiene a questa categoria.
